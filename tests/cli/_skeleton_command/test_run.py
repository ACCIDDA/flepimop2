...
"""
Tests for `SkeletonCommand.run` via the `flepimop2 skeleton` CLI.

These characterize the current scaffolding behavior so it can be refactored
(see #250) without silently changing what `flepimop2 skeleton` produces.
"""
...
import flepimop2.cli._skeleton_command as skeleton_module
from flepimop2.cli._cli import cli
from flepimop2.typing import ExitCode

_TEMPLATE_DIR = Path(skeleton_module.__file__).parent.parent / "templates" / "skeleton"
...
def test_run_scaffolds_full_template_tree(tmp_path: Path) -> None:
    """`skeleton <path>` reproduces the bundled template tree, contents and all."""
...
def test_run_defaults_to_current_directory() -> None:
    """`skeleton` with no path argument scaffolds into the working directory."""
    runner = CliRunner()
    with runner.isolated_filesystem() as filesystem:
        result = runner.invoke(cli, ["skeleton"], catch_exceptions=False)
...
def test_run_dry_run_reports_target_without_writing(tmp_path: Path) -> None:
    """`skeleton --dry-run` reports the target but creates nothing on disk."""
    target = tmp_path / "project"

    result = CliRunner().invoke(
        cli,
        ["skeleton", "--dry-run", "-vv", str(target)],
        catch_exceptions=False,
    )

    assert result.exit_code == ExitCode.OKAY
    assert not target.exists()
    assert "Would create skeleton project at" in result.output
