...
- Loosened `ParameterValue.value` to any Array-API-compliant array (`numpy.ndarray`, `jax.Array`, etc.) by retyping the field via a new `flepimop2.typing.Array` `Protocol` and removing the unconditional `np.asarray(..., dtype=np.float64)` cast in `__post_init__`. Producers are responsible for handing in the correct dtype, which `mypy` already enforces; consumers that require NumPy now coerce explicitly at the engine boundary. This unblocks engine-level `jax.vmap` / `jit` / inference workflows. See [#270](https://github.com/ACCIDDA/flepimop2/issues/270).
